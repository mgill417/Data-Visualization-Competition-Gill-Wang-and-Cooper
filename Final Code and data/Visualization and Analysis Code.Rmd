...
# Logging sales and replacing missing data (0 local sales, from row total summation) as NA
final_datavis_data$log_sales <- log(final_datavis_data$t_local_sales_2022_num)
final_datavis_data[is.na(final_datavis_data) | final_datavis_data=="-Inf"] = NA
final_datavis_data$t_local_sales_2022_num[final_datavis_data$t_local_sales_2022_num==0] = NA


model_1<- lm(log_sales~ num_local_sellers_2022_2023+ MPI_count + PCT_LACCESS_POP10+ t_BIPOC_num_prods_2022 +  rural_RUCA , data= final_datavis_data )
...
```
```{r}
final_datavis_data$cats.num.BIPOC <- cut(final_datavis_data$hispanic_number_producers_2022, 
                          breaks = c(0, 7, 18, 51, 723), 
                          labels = c("Quantile 1: 0-7", "Quantile 2: 8-18", "Quantile 3: 19-51", "Quantile 4: 52- 723"), 
                          right = FALSE)
...
long_caption5<-"RUCA= Rural-Urban Commuting Areas by USDA ERS, where: 1= Metropolitan area core...4= Micropolitan area core...10= Rural area core. Data is originally sourced from the NASS Agricultural Census. "
wrapped_caption5<- str_wrap(long_caption5, width = 80)

ggplot(d, aes(x = x, y = factor(y), fill = factor(y))) + 
  geom_density_ridges(alpha = 0.8, scale = 1) +
  labs(title = "Number of Hispanic Producers by RUCA codes",
       x = "RUCA",
       y = "Num. of Hispanic Producers in 2022", 
       caption = wrapped_caption5) +
  theme_ridges() +   theme(legend.position = "none") +
  scale_fill_viridis_d(name = "Num. of Hispanic Producers in 2022")
```
